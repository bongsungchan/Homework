...
## 출력 위치
- 단계별: `.claude/feature/<phase>/diagram.mmd` + 산출물 요약 `.claude/feature/<phase>/output.md` (+ 렌더 가능 시 `diagram.png`)
- 전체 개요: `.claude/feature/pipeline.mmd`
...
## 렌더 (mmdc 있을 때)
```bash
mmdc -i .claude/feature/<phase>/diagram.mmd -o .claude/feature/<phase>/diagram.png -b white --scale 2
```
